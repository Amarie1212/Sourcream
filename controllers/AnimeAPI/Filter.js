...
exports.index = async (req, res) => {
  try {
    ...
    const url = `animasu.love/pencarian/?genre%5B0%5D=${genre}&status=${status}&tipe=${type}&urutan=${order}&halaman=${page}`;
    ...
    $('.listupd .bsx').each((i, element) => {
      ...
      const slug = $(element).find('a').attr('href')?.replace(/^https?:\/\/(?:v1\.animasu\.top|animasu\.love)\/anime\//, '').replace(/\/$/, '') || '#';
