...
exports.index = async (req, res) => {
  try {
	...
	const url = `animasu.love/daftar-anime/page/${page}/?show=${show}`;
    ...
    $('.listo .bx').each((i, element) => {
      ...
      const slug = $(element).find('.inx h2 a').attr('href')?.replace(/^https?:\/\/(?:v1\.animasu\.top|animasu\.love)\/anime\//, '').replace(/\/$/, '') || '#';
